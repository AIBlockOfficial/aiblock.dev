# Mempool Node
The Mempool node is one half of the very important balance of power in the Zenotta network. While fully distributed mining, as seen in Bitcoin, is ideal for security it is inadequate for other factors such as speed. As a result, we at Zenotta realised that a good compromise is a ring of decentralised Mempool nodes to facilitate transactions, with public, fully distributed Miner nodes still responsible for mining the blocks containing them and for providing the input into the verifiably fair decision-making process for selecting transactions.
