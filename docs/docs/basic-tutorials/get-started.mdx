...
Create an instance of an A-Block wallet can be done in three main ways:
...
- **`mempoolHost`**: The IP or domain of the mempool node you want to connect to. You'll need this to ... ** *required **
...
- `notaryHost`: The IP or domain of the notary node you want to connect to. You'll need this to ...
...
If you have an existing seed phrase and want to generate a wallet from it, you can use `fromMasterKey` or `initFromSeed` methods:
...
1. The user has lost their key-pairs and re-generation is needed by providing the seed phrase.
...
A-Block is a UTXO blockchain, so you can generate a new keypair, for example, by using the `getNewKeypair` method:
